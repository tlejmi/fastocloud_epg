...
int main(int argc, char** argv, char** envp) {
  ...
  // start
  err = common::file_system::create_directory(config.epg_in_path, true);
  if (err) {
    WARNING_LOG() << "Failed create epg_in directory: " << err->GetDescription();
  }
  err = common::file_system::create_directory(config.epg_out_path, true);
  if (err) {
    WARNING_LOG() << "Failed create epg_out directory: " << err->GetDescription();
  }
